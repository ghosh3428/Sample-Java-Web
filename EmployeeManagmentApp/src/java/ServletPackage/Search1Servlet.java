...
@WebServlet(name = "Search1Servlet", urlPatterns = {"/search1Servlet"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) 
        {
            ...
            if(e == null)
            {
                ...
                out.print("<h1 class='text-danger'>Incorrect Employee ID</h1>");
                rd.include(request, response); 
            }
            else
            {
                request.setAttribute("employee", e);
                request.getRequestDispatcher("/employeeupdateform.jsp").forward(request, response);
            }
